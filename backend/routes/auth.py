...
# --- Public Endpoints (Whitelisted Admission / Session) ---
...
def validate_activation_token(token: str, db: Session = Depends(get_db)):
    ...
    entry = db.query(AuthWhitelist).filter(
        AuthWhitelist.token_hash == t_hash,
        AuthWhitelist.status == "pending"
    ).first()
...
def register():
    raise HTTPException(
        status_code=403,
        detail="El registro público está deshabilitado. Se requiere una invitación de acceso para CleanSheet."
    )
...
def login(req: LoginRequest, response: Response, db: Session = Depends(get_db)):
    ...
    whitelist = db.query(AuthWhitelist).filter(
        (AuthWhitelist.user_id == user.id) | (AuthWhitelist.email == clean_email)
    ).first()
...
def logout(response: Response, user: Optional[User] = Depends(get_current_user_optional), db: Session = Depends(get_db)):
    ...
    response.delete_cookie(key="access_token", path="/", httponly=True, secure=COOKIE_SECURE, samesite=COOKIE_SAMESITE)
...
def add_to_whitelist(
    ...
    existing = db.query(AuthWhitelist).filter(AuthWhitelist.email == clean_email).first()
...
def rotate_whitelist_token(
    ...
    entry = db.query(AuthWhitelist).filter(AuthWhitelist.id == id).first()
    if not entry:
